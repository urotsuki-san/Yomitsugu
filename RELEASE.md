...
2026-09-26時点の個人評価用です。一般配布の承認済み版ではありません。今回の利用目的では公開配布用の署名は対象外です。0.2.7では長い英日混在入力、`sannkai` の候補順位を修正し、通知領域アイコンと公開用READMEを更新しました。
...
`scripts/package_release.ps1` で `dist/yomitsugu-0.2.7-preview` を生成します。DLL／実行ファイル、専用エンジン、辞書、モデル、依存ライセンス、SHA-256一覧をまとめます。ZIPの横に同名の `.zip.sha256` を置きます。パッケージ生成は既存フォルダを上書きせず失敗します。

通常のインストールは公開判定と署名を検証します。本版は `public_release_ready=false` なので通常インストールを停止します。開発評価を行う場合だけ、**管理者として開いた64bit PowerShell**で配布フォルダに移動し、次を実行してください。PowerShellではカレントディレクトリのスクリプトに `./` または `.\` を付ける必要があります。
...
6. 別ユーザー／クリーン環境へのインストール、設定画面の視覚・スクリーンリーダー評価、ソース全体のバージョン管理とクリーン環境ビルド。本ワークスペースのルートはGitリポジトリではない。
